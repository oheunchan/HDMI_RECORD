...
#include "stdio.h"
#include "stm32f1xx_it.h"  //oec20260108


extern	int _write(int file, char* p, int len);
extern 	void LED_ON_OFF(u8 onoff);

volatile unsigned char g_tx_vidstable_irq = 0;   //oec20260111
volatile unsigned char g_hdmitx_defer_unmute = 0;   //oec20266011c
...
_CODE HDMITXDEV InstanceData =
{
    ...
    /////////////////////////////////////////////////
    // Device Version/*20230220*/
    /////////////////////////////////////////////////
    0x21,      // BYTE bDeviceVersion ;
    /////////////////////////////////////////////////
    // Interrupt Type
    /////////////////////////////////////////////////
    ...
    INPUT_SIGNAL_TYPE,//24 bit sync seperate
...
static HDMI_VTiming _CODE s_VMTable[] = {
    ...
    { 6,1,720,240,858,262,13500000L,0x00,19,62,57,4,3,15,INTERLACE,Vneg,Hneg},//720x480(I)@60Hz
    { 7,1,720,240,858,262,13500000L,0x00,19,62,57,4,3,15,INTERLACE,Vneg,Hneg},//720x480(I)@60Hz
    { 8,1,720,240,858,262,13500000L,0x00,19,62,57,4,3,15,PROG,Vneg,Hneg},//720x480(I)@60Hz
    { 9,1,720,240,858,262,13500000L,0x00,19,62,57,4,3,15,PROG,Vneg,Hneg},//720x480(I)@60Hz
    ...
    {21,1,720,288,864,312,13500000L,0x00,12,63,69,2,3,19,INTERLACE,Vneg,Hneg},//1440x576(I)@50Hz
    {22,1,720,288,864,312,13500000L,0x00,12,63,69,2,3,19,INTERLACE,Vneg,Hneg},//1440x576(I)@50Hz
    {23,1,720,288,864,312,13500000L,0x00,12,63,69,2,3,19,PROG,Vneg,Hneg},//1440x288@50Hz
    {24,1,720,288,864,312,13500000L,0x00,12,63,69,2,3,19,PROG,Vneg,Hneg},//1440x288@50Hz
    {25,2,720,288,864,312,13500000L,0x00,12,63,69,2,3,19,INTERLACE,Vneg,Hneg},//1440x576(I)@50Hz
    {26,2,720,288,864,312,13500000L,0x00,12,63,69,2,3,19,INTERLACE,Vneg,Hneg},//1440x576(I)@50Hz
    {27,2,720,288,864,312,13500000L,0x00,12,63,69,2,3,19,PROG,Vneg,Hneg},//1440x288@50Hz
    {28,2,720,288,864,312,13500000L,0x00,12,63,69,2,3,19,PROG,Vneg,Hneg},//1440x288@50Hz
    ...
    //20221205
    {0x03,0,3840,2160,5500,2250,297000000L,0x0B,1276,88,296,8,10,72,PROG,Vpos,Hpos},// 3840x2160@24
    {0x02,0,3840,2160,5280,2250,297000000L,0x0B,1056,88,296,8,10,72,PROG,Vpos,Hpos},// 3840x2160@25
    {0x01,0,3840,2160,4400,2250,297000000L,0x0B,176,88,296,8,10,72,PROG,Vpos,Hpos},// 3840x2160@30
    {0x04,0,4096,2160,5500,2250,297000000L,0x0B,1020,88,296,8,10,72,PROG,Vpos,Hpos},// 4096x2160@24
    // VESA mode
...
ULONG VideoPixelClock = 0 ;
BYTE VIC ; // 480p60
BYTE HDMI_VIC = 0;//20221205
...
BYTE ParameterChange = FALSE;/*20230220*/
...
void ConfigSPDInfoFrm();/*20230210*/
void Config_GeneralPurpose_Infoframe(BYTE *p3DInfoFrame);
void ConfigfHdmiVendorSpecificInfoFrame(BYTE _3D_Stru);
void ConfigfHdmiVendorSpecificInfoFrame4k30(void);//20221203
void SetInputColorMode(BYTE input_color_mode);//20221206
void SetOutputColorMode(BYTE output_color_mode);//20221206
BYTE GetOutputColorMode(void);//20221219
BYTE GetInputColorMode(void);/*20230220*/
void SetChangeMode(BYTE on);//20221213
...
// Function Body.
////////////////////////////////////////////////////////////////////////////////
void ResetVideoPixelClock(void)/*20230201*/
{
    VideoPixelClock = 0;
}

void SetInputColorMode(BYTE input_color_mode)
{
    bInputColorMode = input_color_mode;
}

void SetOutputColorMode(BYTE output_color_mode)
{
    bOutputColorMode = output_color_mode;
}

BYTE GetOutputColorMode(void)
{
    return bOutputColorMode;
}

BYTE GetInputColorMode(void)
{
    return bInputColorMode;
}

void SetChangeMode(BYTE on)
{
    if(on){
        bChangeMode =  TRUE;
    }else{
...
void HDMITX_SetOutput()
{
    ...
    BYTE InputSignalType = GetInputSignalType();
    BYTE DeviceID = GetDeviceVersion();/*20230220*/
    HDMITX_DisableAudioOutput();
	HDMITX_EnableHDCP(FALSE);

    if( TMDSClock>180000000L )
    {
        if(DeviceID == Device_IT66121){/*20230220*/
            HDMITX_DEBUG_PRINTF(("Device_IT66121 Not Support PCLK High !!\r\n"));
            return;
        }
        level = PCLK_HIGH ;
    }
    else if(TMDSClock>80000000L)
    ...
    /*20230605*//*causing PC timing no display, mark off*//*start*/
    /*20230201*//*issue: avoid vid out before frontend ready, so reset VID when TMDS = 0 to re-detect correct TMDS*/
    // if(TMDSClock == 0)
    // {
    //     HDMITX_DEBUG_PRINTF(("TMDSClock == 0 !!\n"));
    //     HDMITX_SetI2C_Byte(REG_TX_SW_RST,B_HDMITX_VID_RST,B_HDMITX_VID_RST);
    //     delay1ms(1);
    //     HDMITX_SetI2C_Byte(REG_TX_SW_RST,B_HDMITX_VID_RST,0);
    //     return;
    // }
    /*20230605*//*causing PC timing no display, mark off*//*end*/
	...
	setHDMITX_VideoSignalType(InputSignalType);
    #ifdef SUPPORT_SYNCEMBEDDED
	if(InputSignalType & T_MODE_SYNCEMB)
	{
	    setHDMITX_SyncEmbeddedByVIC(VIC,InputSignalType);
	}
    #endif

    HDMITX_DEBUG_PRINTF(("level = %d,bInputColorMode=%x,bOutputColorMode=%x,bHDMIMode=%x,VIC=%x\r\n",(int)level,(int)bInputColorMode,(int)bOutputColorMode,(int)bHDMIMode,(int)VIC));
    HDMITX_DEBUG_PRINTF(("bHDMIMode=%x,VIC=%x\r\n",(int)bHDMIMode,(int)VIC));
    ...
    {
        if(HDMI_VIC != 0x00){
            ConfigfHdmiVendorSpecificInfoFrame4k30();
        }else{
...
/* =========================================================
 * Hotplug Robust Fix (2026-01-11)
 *  - HPD debounce + unplug cooldown (non-blocking)
 *  - Clear packet/video status (reg0x0C) on every plug sequence
 *  - Start output only after RXSENSE stable + VID_STABLE (IRQ or poll)
 * ========================================================= */

static void HDMITX_ClearPktVidStatus(void)
{
    /* Force bank0 for reg0x0C */
    HDMITX_SetI2C_Byte(0x0F, 0x03, 0x00);

    /* reg0x0C: [6]VidStableClr [5]PktMpgClr [3]PktAudClr [2]PktAVIClr [1]Pkt3DClr [0]VidUnstableClr */
    HDMITX_WriteI2C_Byte(0x0C, 0x6F);
    HDMITX_WriteI2C_Byte(0x0C, 0x00);
}
...
void HDMITX_DevLoopProc(void)
{
    /* plug state:
     * 0: idle/off
     * 1: wait HPD stable + unplug cooldown
     * 2: init once (PowerOn -> SWRST -> EDID -> SetOutput)
     * 3: running
     */
    static unsigned char s_plug_state = 0;
    static unsigned int  s_hpd_on_ms = 0;
    static unsigned int  s_unplug_ms = 0;
    static unsigned int  s_last_hpd_edge_ms = 0;
    ...
    now_ms = (unsigned int)HAL_GetTick();

    /* -------------------------
     * HPD OFF : immediate cleanup
     * ------------------------- */
    if (HPDChangeStatus && (HPDStatus == FALSE))
    {
        if ((now_ms - s_last_hpd_edge_ms) < 10)
        {
            HPDChangeStatus = FALSE;
            return;
        }
        s_last_hpd_edge_ms = now_ms;

        printf("[HPD] OFF t=%u\r\n", now_ms);

#ifdef _SUPPORT_HDMI_REPEATER_
        IT680X_SetUpstream_HPD(FALSE);
#endif
#ifdef _SUPPORT_HDCP_REPEATER_
        ...
        /* clear pkt/vid status for next plug */
        HDMITX_ClearPktVidStatus();
        ...
        bChangeMode  = FALSE;
        bChangeAudio = FALSE;
        g_tx_vidstable_irq = 0;
        g_hdmitx_defer_unmute = 0;
    ...
    /* -------------------------
     * HPD ON edge : enter debounce state
     * ------------------------- */
    if (HPDChangeStatus && (HPDStatus == TRUE))
    {
        if ((now_ms - s_last_hpd_edge_ms) < 10)
        {
            HPDChangeStatus = FALSE;
    ...
    /* -------------------------
     * Plug-on state machine
     * ------------------------- */
    if (HPDStatus)
    {
        /* state 1: HPD stable wait + unplug cooldown */
        if (s_plug_state == 1)
        {
            if ((now_ms - s_hpd_on_ms) < 80)   /* HPD settle */
            {
                return;
            }
            if ((now_ms - s_unplug_ms) < 250) /* unplug cooldown */
            {
                return;
            }
            s_plug_state = 2;
        ...
        /* state 2: init once (do NOT depend on RXSENSE bit to avoid no-display) */
        if (s_plug_state == 2)
        {
            int edid_ok = 0;
            unsigned char i;
            ...
            /* Full SW reset (video/audio/hdcp/ref) */
            ...
            delay1ms(2);
            HDMITX_WriteI2C_Byte(REG_TX_SW_RST, 0x00);

            /* Clear pkt/vid status to prevent CSC/AVI residue */
            HDMITX_ClearPktVidStatus();

            /* default outputs (safe RGB) */
            bOutputColorMode = OUTPUT_COLOR_MODE;
            ...
            /* EDID retry (fast hotplug can glitch DDC) */
            for (i = 0; i < 3; i++)
            {
                ParseEDID();
                if (RxCapability.ValidHDMI || RxCapability.ValidCEA)
                {
                    edid_ok = 1;
                    break;
                }
            ...
            if (edid_ok && RxCapability.ValidHDMI)
            {
                bHDMIMode = TRUE;
...
#ifdef SET_OUTPUT_COLORMODE_BY_EDID
                /* keep RGB as default; only switch if EDID explicitly supports */
            ...
            {
                /* treat as DVI when EDID unstable */
                bHDMIMode = FALSE;
                bAudioEnable = FALSE;
                OutputColorDepth = B_TX_CD_NODEF;
#ifdef SUPPORT_HDCP
            ...
            /* One-shot SetOutput : allow internal unmute */
            g_hdmitx_defer_unmute = 0;
            HDMITX_SetOutput();
            ...
            TxHDCP_chg(TxHDCP_Off);
#endif
            IT680X_SetUpstream_HPD(TRUE);
#endif

            /* extra AVI resend to recover from fast re-plug */
            if (bHDMIMode)
            {
                ConfigAVIInfoFrame(VIC, pixelrep);
                delay1ms(20);
        ...
        /* state 3: normal running path (keep original behavior) */
        if (s_plug_state == 3)
        {
        ...
        if( ScanMode == s_VMTable[i].ScanMode )
        {
            ...
            if( hit == 2 ) // match scan mode and v-total
            {
                if( DIFF(HTotal, s_VMTable[i].HTotal) < 40 )
                {
                    ...
                    pclkDiff = ((pclkDiff*10) / s_VMTable[i].PCLK)*10;/*20230703*//*overflow issue*/
...
void HDMITX_MonitorInputVideoChange()
{
    ...
    bInputVideoSignalType = GetInputSignalType();
    currPCLK = CalcPCLK() ;
    ...
    if( bChangeMode )
    {
        ...
        #endif
        /*20221219*/
        if((currVTotal>2000) && (i >= 0))
        {
            VIC = 0;
            HDMI_VIC = s_VMTable[i].VIC;
        ...
        else
        {
            ...
            HDMI_VIC = 0;
            pixelrep = 0;
            VideoPixelClock = currPCLK ;/*20230615*//*for timing not in table*/
        }
        HDMITX_ConfigAspecColorimetry(VIC, HDMI_VIC);
    }
...
/*20230608*//*AVI aspec ratio*/
void HDMITX_ConfigAspecColorimetry(BYTE input_VIC, BYTE input_HDMI_VIC)
{
    if(input_HDMI_VIC!=0){
        aspec = HDMI_16x9 ;
...
void HDMITX_ChangeDisplayOption(HDMI_Video_Type OutputVideoTiming, HDMI_OutputColorMode OutputColorMode)
{
    ...
    */
    ...
    case HDMI_4k30:
		...
		printf("HDMITX_ChangeDisplayOption(HDMI_4k30,HDMI_RGB444) ;\r\n") ;
        break;
    case HDMI_4k24_SMPTE:
        VIC = 0 ;
        VideoPixelClock = 297000000 ;
        pixelrep = 0 ;
        aspec = HDMI_16x9 ;
...
void ConfigAudioInfoFrm()
{
    ...
    AudioInfo->pktbyte.AUD_DB[0] = OUTPUT_CHANNEL-1 ;/*20230605*//*Audio channel setting error issue.*/
    for( i = 1 ;i < AUDIO_INFOFRAME_LEN ; i++ )
    {
        AudioInfo->pktbyte.AUD_DB[i] = 0 ;
    }
    /*20230605*//*audio channel count error issue*//*start*/
    ...
    /*20230605*//*audio channel count error issue*//*end*/
    HDMITX_EnableAudioInfoFrame(TRUE, (unsigned char *)AudioInfo);
}
/*20230210*/
void ConfigSPDInfoFrm()
{
    int i ;

    SPD_InfoFrame *SPDInfo ;
    SPDInfo = (SPD_InfoFrame *)CommunBuff ;

    HDMITX_DEBUG_PRINTF(("ConfigSPDInfoFrm(%d)\r\n",0));

    SPDInfo->pktbyte.SPD_HB[0] = SPD_INFOFRAME_TYPE ;
    SPDInfo->pktbyte.SPD_HB[1] = SPD_INFOFRAME_VER ;
    SPDInfo->pktbyte.SPD_HB[2] = SPD_INFOFRAME_LEN ;
    for(i=0 ; i<(SPD_INFOFRAME_LEN-1) ; i++){
        SPDInfo->pktbyte.SPD_DB[i] = 0;
    }
    SPDInfo->pktbyte.SPD_DB[0] = 'i';
    SPDInfo->pktbyte.SPD_DB[1] = 'T';
    SPDInfo->pktbyte.SPD_DB[2] = 'E';
    SPDInfo->pktbyte.SPD_DB[3] = 'P';
    SPDInfo->pktbyte.SPD_DB[4] = 'R';
    SPDInfo->pktbyte.SPD_DB[5] = 'O';
    SPDInfo->pktbyte.SPD_DB[6] = 'D';
    SPDInfo->pktbyte.SPD_DB[8] = 'S';
    SPDInfo->pktbyte.SPD_DB[9] = 'A';
    SPDInfo->pktbyte.SPD_DB[10] = 'M';
    SPDInfo->pktbyte.SPD_DB[11] = 'P';
    SPDInfo->pktbyte.SPD_DB[12] = 'L';
    SPDInfo->pktbyte.SPD_DB[13] = 'E';
    SPDInfo->pktbyte.SPD_DB[24] = 0x01;
    HDMITX_EnableSPDInfoFrame(TRUE, (unsigned char *)SPDInfo);
}
/*******/
void ConfigfHdmiVendorSpecificInfoFrame(BYTE _3D_Stru)
{
...
void ConfigfHdmiVendorSpecificInfoFrame4k30(void)
{
    VendorSpecific_InfoFrame *VS_Info;
    ...
    VS_Info->pktbyte.VS_DB[1] = 0x0C;
    VS_Info->pktbyte.VS_DB[2] = 0x00;
    VS_Info->pktbyte.VS_DB[3] = 0x20;
    VS_Info->pktbyte.VS_DB[4] = HDMI_VIC;
    VS_Info->pktbyte.VS_DB[5] = 0x00;
    HDMITX_EnableVSInfoFrame(TRUE,(BYTE *)VS_Info);
}
...
BYTE ParseEDID()
{
    ...
    else {	printf("\r\nEDID Header %x %x %x %x %x %x %x %x\r\n",EDID_Buf[0],EDID_Buf[1],EDID_Buf[2],EDID_Buf[3],EDID_Buf[4],EDID_Buf[5],EDID_Buf[6],EDID_Buf[7]);}
    ...
    */
    ...
    for( i = 1 ; i <= BlockCount ; i++ )
    {
        ...
        for( j = 0 ; j < 128 ; j++ )
        {
            EDID_DEBUG_PRINTF(("[EDID] %02X%c",(int)EDID_Buf[j],(7 == (j&7))?'\r\n':'\r\n'));
//		EDID_DEBUG_PRINTF(("[EDID %d] %02X%c",j,(int)EDID_Buf[j],(7 == (j&7))?'\r\n':' '));
...
//	printf("!!Vedner ID : %x %x\r\n",pCEAEDID[7],pCEAEDID[8]);
//	printf("!!Vedner ID : %x %x\r\n",pCEAEDID[7],pCEAEDID[8]);
